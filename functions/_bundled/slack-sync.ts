...
async function fetchChannels(token: string) {
  ...
  do {
    const params: Record<string, string> = { types: 'public_channel', limit: '200', exclude_archived: 'true' };
  ...
  try {
    ...
    for (const ch of slackChannels) {
      await admin.database.from('channels').upsert({
        workspace_id: workspace.id, slack_channel_id: ch.id, name: ch.name,
        topic: ch.topic?.value || null, member_count: ch.num_members || 0,
      }, { onConflict: 'workspace_id,slack_channel_id' });
    }

    // Get channel UUID map
    const { data: dbChannels } = await admin.database
      .from('channels').select('id, slack_channel_id').eq('workspace_id', workspace.id);
    const channelMap = new Map((dbChannels ?? []).map((c: any) => [c.slack_channel_id, c.id]));

    // Sync messages
    const oldestTs = lastSyncedAt ? (new Date(lastSyncedAt).getTime() / 1000).toString() : undefined;
    let totalMessages = 0;
    const channelsToSync = slackChannels.slice(0, 20);
      ...
      try {
        ...
        for (const msg of messages) {
          ...
          await admin.database.from('messages').upsert({
            workspace_id: workspace.id, channel_id: channelUuid, slack_ts: msg.ts,
            author_slack_id: msg.user || 'unknown', author_name: msg.user || 'Unknown',
            content: msg.text, thread_ts: msg.thread_ts !== msg.ts ? msg.thread_ts : null,
            reply_count: msg.reply_count || 0,
            reaction_count: msg.reactions ? msg.reactions.reduce((sum: number, r: any) => sum + (r.count || 0), 0) : 0,
        ...
        if (messages.length > 0) {
          ...
          await admin.database.from('channels')
            .update({ last_message_at: new Date(latestTs * 1000).toISOString() }).eq('id', channelUuid);
        }
      } catch (err) { console.error(`Failed to sync channel ${ch.name}:`, err); }
      await new Promise((resolve) => setTimeout(resolve, 200));
    }

    // Update watermark
    await admin.database.from('workspaces')
      .update({ last_synced_at: new Date().toISOString() }).eq('id', workspace.id);

    return new Response(JSON.stringify({ success: true, channels_synced: channelsToSync.length, messages_synced: totalMessages }),
